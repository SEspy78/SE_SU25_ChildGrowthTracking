...
const FacilityDashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      console.log("API Response:", res); // Ghi log để debug
  ...
  useEffect(() => {
    fetchDashboardData(); // Gọi lần đầu khi component mount
  ...
  // Dữ liệu cho biểu đồ tròn (appointmentStats)
  const pieData = dashboardData?.appointmentStats
    ? [
        { type: "Chờ duyệt", value: dashboardData.appointmentStats.pending || 0 },
        { type: "Hoàn thành", value: dashboardData.appointmentStats.completed || 0 },
        { type: "Đã duyệt", value: dashboardData.appointmentStats.approval || 0 },
        { type: "Đã hủy", value: dashboardData.appointmentStats.cancelled || 0 },
        { type: "Đã thanh toán", value: dashboardData.appointmentStats.paid || 0 },
      ].filter((item) => item.value > 0) // Loại bỏ giá trị 0 để tránh biểu đồ trống
    : [];
  ...
  const pieConfig = {
    ...
    color: ["#FF6B6B", "#4CAF50", "#2196F3", "#FF9800", "#4BC0C0"],
  };
          ...
          <Col xs={24} sm={12} md={8}>
            <Card className="bg-red-50 border-red-200">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-red-600">Đơn hàng đang chờ</p>
                  <p className="text-2xl font-bold text-red-900">{dashboardData?.revenueStats?.pendingOrdersCount || 0}</p>
                </div>
              </div>
            </Card>
          </Col>
          ...
          <Col xs={24} md={12}>
            <Card className="bg-gray-50 border-gray-200">
              {pieData.length > 0 ? (
                ...
                <p className="text-center text-gray-500">Không có dữ liệu cuộc hẹn để hiển thị.</p>
              )}
            </Card>
          </Col>
        </Row>
